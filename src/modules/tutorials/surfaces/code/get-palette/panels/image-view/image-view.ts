...
export class ImageView extends Panel<ImageViewData> {
  ...
  private blit () {
    if (this.preview) {
      this.preview.clear()
      if (this._image) { 
        const rect = this._image.rect.move(this.offset).scalesize(this.zoom)
  ...
  private panelCallback (actionName: ImageViewAction, args: any) {
    switch (actionName) {
      ...
      case "moving": {
        if (args && args.down) {
          this.offset.shiftSelf(args.shift)
          this.blit()
        }
        break
      }
      case "zoom":
        ...
        this.zoom = newZoom
        this.offset.x = pos.x - worldX * newZoom
        this.offset.y = pos.y - worldY * newZoom
        this.blit()
      break
      case "upload": {
  ...
  private fitContain () {
    ...
    this.zoom = zoom
    this.offset.moveSelf(this.preview.width / 2 - imageCenterX * zoom,this.preview.height / 2 - imageCenterY * zoom)
  }
}